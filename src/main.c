...
const uint16_t defaultCCR1 = 20000;
...
const uint16_t zc_required = 2;
volatile uint16_t zc_pass;
// for test
void comparator_zc_isr()
{
  while (zc_pass++ < zc_required) {
...
int main()
{
  ...
  for (uint8_t i = 0; i < 6; i++) {
    start_motor();
    advance = i;
    for (uint32_t i = 0; i < 400000; i++) { float a = 0.6*9; }
    stop_motor();
  }
  bridge_disable();
  stop_motor();
